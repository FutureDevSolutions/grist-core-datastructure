...
export class VirtualDoc extends DisposableWithEvents implements GristDoc {
  ...
  /**
   * Returns the numeric section ID for the main section of a given table.
   * Useful when building a custom layoutSpec.
   */
  public getSectionId(tableId: string): number {
    return this.getMainSectionRec(tableId).id.peek();
  }

  /**
   * Sets the layoutSpec on the "main" view and activates it. Call after all addTable() calls.
   * Use parentViewId: "main" in TableSpec so sections are already in the main view.
   * The spec uses section IDs as leaf values (from getSectionId()):
   *   { children: [{ leaf: trigLogId }, { children: [{ leaf: emailId }, { leaf: webhookId }] }] }
   */
  public setLayoutSpec(spec: BoxSpec) {
    this.docData.receiveAction([
      "UpdateRecord", "_grist_Views", "main" as any as number, {
...
export interface TableSpec {
  ...
  initialFocus?: boolean;
  defaultWidth?: number;
  /** If provided, attach the section to this view ID instead of creating a private view.
   * Use "main" for the default view. */
...
function generateInitialActions(tabDef: TableSpec): DocAction[] {
  ...
  const viewId = tabDef.parentViewId ?? VirtualId();
  const sectionRowId = VirtualId();
  const ownView = !tabDef.parentViewId;
  ...
  return [
    ...
    ...(ownView ? [[
      // Add view instance (skipped when attaching to an existing view).
