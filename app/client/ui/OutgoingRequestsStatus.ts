...
export function buildOutgoingRequestsSummary(result: BootProbeResult | undefined): DomContents {
  ...
  // Render the probe's posture verbatim; never recompute from status/proxy.
  switch (readDetails(result)?.posture ?? "inactive") {
    case "filtered": return cssValueLabel(cssHappyText(t("filtered")));
    case "unfiltered": return cssValueLabel(cssDangerText(t("unfiltered")));
    case "review": return cssValueLabel(cssDangerText(t("review")));
    case "bypassed": return cssValueLabel(cssDangerText(t("bypassed")));
    default: return cssValueLabel(cssGrayed(t("not configured")));
  }
}
...
export function buildOutgoingRequestsPanel(result: BootProbeResult | undefined): DomContents {
  ...
  return cssOutReqContainer(
    _buildBanner(details?.posture ?? "inactive"),
...
Here's how each one is set up right now.")),
    ...
    _buildProxyBox(details?.proxy),
  );
}

// CSS variant on cssWell that colours each banner. Several postures share a
// colour (e.g. "review" and "bypassed" are both warning-amber).
...
// One banner per posture; the client renders this verbatim from the probe's
// posture and does not re-derive it from status/proxy fields.
const BANNER_BY_POSTURE: Record<OutgoingRequestsPosture, BannerCopy> = {
  unfiltered: {
    kind: "error",
    icon: "Warning",
...
including addresses on your internal network. Configure a filtering proxy to close this off.",
  ...
  bypassed: {
    kind: "warning",
    icon: "Warning",
    title: "URL filtering is switched off",
    body: "Outgoing requests skip the URL filter because GRIST_PROXY_FOR_UNTRUSTED_URLS is set to \
\"direct\". Traffic goes straight out, including to addresses on your internal network.",
  },
  filtered: {
    kind: "success",
    icon: "Tick",
...
function _buildBanner(posture: OutgoingRequestsPosture): DomContents {
  const copy = BANNER_BY_POSTURE[posture];
  return cssWell(cssWell.cls(`-${copy.kind}`),
...
function _buildFeatureRow(check: OutgoingRequestsFeatureCheck): DomContents {
  ...
  const meta = check.wildcardAllowed ?
    ...
    (check.allowedDomains && check.allowedDomains.length > 0 ?
      ...
      undefined);
  // Only claim the proxy makes wildcard webhooks safe when it is actually
  // filtering them ("on-proxied"); not for "on-direct"/"on-unproxied".
  const wildcardNote = (check.state === "on-proxied" && check.wildcardAllowed) ?
